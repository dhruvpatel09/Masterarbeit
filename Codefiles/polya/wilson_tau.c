...
#define polya_RELEASE "wilson_tau 0.1"
...
static struct
{
   ...
   int nc_last;
   int nc_step;
   int tau;      /* temporal extent in lattice units */
   int mu;       /* direction of long side; static-potential case: mu=0 */
} params;
...
static void write_file_head(qcd_geometry *geo)
{
   ...
   error_root(bdio_write(str, len, fbdio)!=len,1,
               "write_file_head [polya.c]",
               "Incorrect write count");
   sprintf(str,"CORR_NAME=W_mu_nu_a_tau_avg_nu");
   len = strlen(str)+1;
   error_root(bdio_write(str, len, fbdio)!=len,1,
               "write_file_head [polya.c]",
               "Incorrect write count");
   ...
   error_root(bdio_write(str, len, fbdio)!=len,1,
               "write_file_head [polya.c]",
               "Incorrect write count");
   sprintf(str,"D0_NAME=avg_nu");
   len = strlen(str)+1;
   error_root(bdio_write(str, len, fbdio)!=len,1,
               "write_file_head [wilson_tau.c]",
               "Incorrect write count");
   sprintf(str,"MU=%d",params.mu);
   len = strlen(str)+1;
   error_root(bdio_write(str, len, fbdio)!=len,1,
               "write_file_head [wilson_tau.c]",
               "Incorrect write count");
   sprintf(str,"TAU=%d",params.tau);
   len = strlen(str)+1;
   error_root(bdio_write(str, len, fbdio)!=len,1,
               "write_file_head [wilson_tau.c]",
               "Incorrect write count");
   sprintf(str,"SPATIAL_EXTENT=a");
   len = strlen(str)+1;
   error_root(bdio_write(str, len, fbdio)!=len,1,
               "write_file_head [wilson_tau.c]",
               "Incorrect write count");
...
static void write_data()
{
   ...
   iw+= bdio_write_f64(&tmp,sizeof(double),fbdio);
   nw = 2*sizeof(double);
   iw+=bdio_write_f64(data.corr, sizeof(double)*2,fbdio);
   nw+=sizeof(double)*2;
...
void read_dirs(void)
{
   if (myid==0)
   {
      ...
      qcd_findOpenQCDSection("Wilson loop");
      qcd_readOpenQCDLine("tau","%d",&(params.tau));
      qcd_readOpenQCDLineOpt("mu","0","%d",&(params.mu));

      error_root((params.nc_last<params.nc_first)||
                 (params.nc_step<1)||
                 (((params.nc_last-params.nc_first)%params.nc_step)!=0),1,
                 "read_dirs [wilson_tau.c]","Improper configuration range");
      error_root(params.tau<1,1,"read_dirs [wilson_tau.c]",
                 "tau must be a positive integer");
      error_root((params.mu<0)||(params.mu>3),1,"read_dirs [wilson_tau.c]",
                 "mu must be 0,1,2,or 3");
   }
   ...
   MPI_Bcast(&(params.tau),1,MPI_INT,0,MPI_COMM_WORLD);
   MPI_Bcast(&(params.mu),1,MPI_INT,0,MPI_COMM_WORLD);
}
...
static void setup_files(void)
{
   ...
   error_root(qcd_nameSize("%s/%s.wilson_tau.bdio~",params.dat_dir,params.outbase)
   ...
   error_root(qcd_nameSize("%s/%s.wilson_tau.log~",params.log_dir,params.outbase)
              >=qcd_MAX_STRING_LENGTH,
              1,"setup_files [polya.c]","log_dir name is too long");

   sprintf(params.log_file,"%s/%s.wilson_tau.log",params.log_dir,params.outbase);
   sprintf(params.end_file,"%s/%s.wilson_tau.end",params.log_dir,params.outbase);
   sprintf(params.dat_file,"%s/%s.wilson_tau.bdio",params.dat_dir,params.outbase);
...
static void check_dat(int *L, int *P)
{
   ...
   if(myid==0)
   {
      ...
      error_root(strcmp(substr,"W_mu_nu_a_tau_avg_nu")!=0,1,"check_dat [polya.c]",
                        "Unexpected CORR_NAME");
      i=find_val_int(str,"NDIM=");
      error_root(i!=1,1,"check_dat [polya.c]",
                        "Unexpected NDIM");
      i=find_val_int(str,"D0=");
      error_root(i!=1,1,"check_dat [wilson_tau.c]",
                        "Unexpected D0");
      substr=find_val(str,"DATATYPE=");
      error_root(strcmp(substr,"complex")!=0,1,"check_dat [wilson_tau.c]",
                        "Unexpected DATATYPE");
      i=find_val_int(str,"MU=");
      error_root(i!=params.mu,1,"check_dat [wilson_tau.c]",
                        "Cannot append: mu mismatch");
      i=find_val_int(str,"TAU=");
      error_root(i!=params.tau,1,"check_dat [wilson_tau.c]",
                        "Cannot append: tau mismatch");
         ...
         (fbdio->state==BDIO_R_STATE || fbdio->state==BDIO_H_STATE))
      {
         ...
         error_root(bdio_get_ruinfo(fbdio)!=5,1,"check_data [polya.c]",
                           "Not a valid %s file", polya_RELEASE);
         error_root((2+2)*8!=bdio_get_rlen(fbdio),1,
                     "check_data [polya.c]",
         ...
         error_root(i!=8,1,"check_data [polya.c]",
                           "Could not read from data file");
         error_root(((int) d)!=0,1,"check_data [wilson_tau.c]",
                           "Unexpected CORR_ID in data record");
         i=bdio_seek_record(fbdio);
      }
      if(nc>0)
...
static void read_infile(int argc, char *argv[], int *L, int *P)
{
   ...
   if(myid==0)
   {
      ...
      error_root((ifile==0)||(ifile==(argc-1)),1,"read_infile [polya.c]",
                 "Syntax: wilson_tau -i <input file> [-a]");
   ...
   error_root(params.tau>=L[params.mu],1,"read_infile [wilson_tau.c]",
              "tau must be smaller than the global extent L[mu]");
   ...
   }else
   {
      if(myid==0)
      {
         ...
         printf("ensemble: %s\n",params.nbase);
         printf("output:   %s\n",params.outbase);
         printf("Wilson-loop direction mu: %i\n",params.mu);
         printf("Temporal extent tau/a:    %i\n",params.tau);
         printf("Spatial extent:           one lattice spacing a\n");
...
{
   /*
    * Computes a rectangular Wilson loop with long side tau in direction mu
    * and short side one lattice spacing a in each transverse direction nu!=mu.
    *
    * W_mu(tau,a) = average_{x,nu!=mu} (1/3) Tr[
    *    L_mu(x,tau) U_nu(x+tau*mu)
    *    L_mu^dagger(x+nu,tau) U_nu^dagger(x)
    * ]
    *
    */
   qcd_geometry *geo = u->geo;
   qcd_gaugeTransformation line, next, shift, shnext;
   qcd_complex_16 A[3][3], B[3][3], C[3][3], trC;
   qcd_complex_16 local_sum, global_sum;
   int mu = params.mu;
   int tau = params.tau;
   int nu, step, nnu;
   long l, lp;
   ...
   if(qcd_initGaugeTransformation(&line, geo) ||
      qcd_initGaugeTransformation(&next, geo) ||
      qcd_initGaugeTransformation(&shift, geo) ||
      qcd_initGaugeTransformation(&shnext, geo))
   {
      fprintf(stderr,"process %i: out of memory for Wilson-loop transporters.\n",myid);
      MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
   }

   /*
    * Build L_mu(x,tau) recursively:
    * L_0(x)=1,
    * L_{n+1}(x)=U_mu(x) L_n(x+mu).
    */
   for(l=0; l<geo->lV; l++)
      qcd_unit3x3(line.D[l]);
   line.BPup2date = 0;
   line.BMup2date = 0;

   for(step=0; step<tau; step++)
   {
      qcd_communicateTransformationM(&line);
      qcd_waitall(geo);
      for(l=0; l<geo->lV; l++)
      {
         lp = geo->plus[l][mu];
         qcd_mul3x3(next.D[l], u->D[l][mu], line.D[lp]);
      }
      for(l=0; l<geo->lV; l++)
         qcd_copy3x3(line.D[l], next.D[l]);
      line.BPup2date = 0;
      line.BMup2date = 0;
   }

   /* Communicate L_mu once so L_mu(x+nu,tau) is available for nu!=mu. */
   qcd_communicateTransformationM(&line);
   qcd_waitall(geo);
   ...
   for(nu=0; nu<4; nu++)
   {
      ...
      /* Build shifted spatial link U_nu(x+tau*mu). */
      for(l=0; l<geo->lV; l++)
         qcd_copy3x3(shift.D[l], u->D[l][nu]);
      shift.BPup2date = 0;
      shift.BMup2date = 0;

      for(step=0; step<tau; step++)
      {
         qcd_communicateTransformationM(&shift);
         qcd_waitall(geo);
         for(l=0; l<geo->lV; l++)
         {
            lp = geo->plus[l][mu];
            qcd_copy3x3(shnext.D[l], shift.D[lp]);
         }
         for(l=0; l<geo->lV; l++)
            qcd_copy3x3(shift.D[l], shnext.D[l]);
         shift.BPup2date = 0;
         shift.BMup2date = 0;
      }
      ...
      for(l=0; l<geo->lV; l++)
      {
         ...
         qcd_mul3x3(A, line.D[l], shift.D[l]);
         qcd_mulAdjoint3x3(B, A, line.D[lp]);
         qcd_mulAdjoint3x3(C, B, u->D[l][nu]);
         trC = qcd_trace3x3(C);
   ...
   MPI_Allreduce(&(local_sum.re), &(global_sum.re), 2, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

   /*
    * Normalize by volume, color trace, and number of transverse directions.
    * On a unit gauge field this gives W=1.
    */
   corr[0] = global_sum.re / ((double)geo->V * 3.0 * (double)nnu);
   corr[1] = global_sum.im / ((double)geo->V * 3.0 * (double)nnu);

   qcd_destroyGaugeTransformation(&line);
   qcd_destroyGaugeTransformation(&next);
   qcd_destroyGaugeTransformation(&shift);
   qcd_destroyGaugeTransformation(&shnext);
}
...
int main(int argc,char* argv[])
{
   ...
   data.corr = malloc(2*sizeof(double));
   if(data.corr==NULL)
   {
   ...
   for(nc=params.nc_first; nc<=params.nc_last; nc+=params.nc_step)
   {
      ...
      if(myid==0)
      {
         printf("Re W_mu(tau,a)=% .12e  Im W_mu(tau,a)=% .12e\n",data.corr[0],data.corr[1]);
	 fflush(stdout);
      }
