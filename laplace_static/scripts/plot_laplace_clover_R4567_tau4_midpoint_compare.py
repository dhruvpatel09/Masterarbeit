...
lines = [load_line(R, 4) for R in [4, 5, 6, 7]]
...
for line in lines:
    ...
    ax.errorbar(
        ...
        marker=markers[R], linestyle="-", capsize=3,
...
for R, alpha in [(4, 0.30), (5, 0.42), (6, 0.55), (7, 0.70)]:
...
ax.set_title(r"Midpoint-centered axial $\rho_S$: $R/a=4,5,6,7$, $\tau/a=4$")
ax.grid(True, alpha=0.3)
ax.legend()

fig.tight_layout()
out = outdir / "laplace_clover_R4567_tau4_rho_S_axis_profile_midpoint_axisavg_jk_ratio_compare.png"
fig.savefig(out, dpi=300)
plt.close(fig)

combined = pd.concat(lines, ignore_index=True)
csv = "results/laplace_clover_R4567_tau4_rho_S_axis_profile_midpoint_axisavg_jk_ratio_compare.csv"
