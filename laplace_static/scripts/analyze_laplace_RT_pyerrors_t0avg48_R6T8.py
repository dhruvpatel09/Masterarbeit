...
JK_FILE = (
    RESULTS /
    "laplace_Veff_R0-6_T1-7_t0avg48_n1-100_jackknife.csv"
)

OUT_L = (
    RESULTS /
    "spatial_avg_RT_R0-6_T1-8_t0avg48_n1-100_pyerrors.csv"
)
...
OUT_CMP = (
    RESULTS /
    "laplace_Veff_R0-6_T1-7_t0avg48_n1-100_error_comparison.csv"
)

ENSEMBLE = "Em1p4_laplace_t0avg48_R6T8"
S_VALUE = 2.0

EXPECTED_CFGS = list(range(1, 101))
EXPECTED_T = list(range(1, 9))
EXPECTED_R = list(range(0, 7))
...
def validate_input(df):
    ...
    if len(df) != 5600:
        raise RuntimeError(
            f"Expected 5600 rows, found {len(df)}"
        )
...
def main():
    ...
    plt.xlabel("T/a")
    plt.ylabel("Re L(R,T)")
    plt.title("Laplace correlator: all-t0 temporal average")
    plt.xticks(EXPECTED_T)
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(
        FIGDIR / "laplace_ReL_vs_T_pyerrors_t0avg48_R6T8.png",
        dpi=200,
    )
    plt.close()
    ...
    plt.title("Laplace effective potential: all-t0 temporal average")
    plt.xticks(EXPECTED_T[:-1])
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(
        FIGDIR / "laplace_Veff_vs_T_pyerrors_t0avg48_R6T8.png",
        dpi=200,
    )
    plt.close()
    ...
    plt.xlabel("R/a")
    plt.ylabel("a Veff(R,T)")
    plt.title("Laplace effective potential versus separation (all-t0 average)")
    plt.xticks(EXPECTED_R[1:])
    plt.grid(True, alpha=0.3)
    plt.legend(ncol=2)
    plt.tight_layout()
    plt.savefig(
        FIGDIR / "laplace_Veff_vs_R_pyerrors_t0avg48_R6T8.png",
        dpi=200,
    )
    plt.close()
    ...
    plt.title("Relative uncertainty of Veff (all-t0 temporal average)")
    plt.xticks(EXPECTED_T[:-1])
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(
        FIGDIR /
        "laplace_Veff_relative_error_pyerrors_t0avg48_R6T8.png",
        dpi=200,
    )
    plt.close()
    ...
    for R in EXPECTED_R[1:]:
        ...
        plt.savefig(
            FIGDIR /
            f"laplace_Veff_pyerrors_zoom_R{R}_t0avg48_R6T8.png",
            dpi=200,
        )
        plt.close()
        ...
        plt.savefig(
            FIGDIR /
            f"laplace_Veff_methods_R{R}_t0avg48_R6T8.png",
            dpi=200,
        )
        plt.close()
