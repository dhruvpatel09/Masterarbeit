...
rm -f \
  ...
  build/test_laplace_tiny_open.o

mpicc ${CFLAGS} \
  -c src/test_laplace_tiny_open.c \
  -o build/test_laplace_tiny_open.o

mpicxx -o build/test_laplace_tiny_open \
  build/test_laplace_tiny_open.o \
  ${LDFLAGS} \
  ${H5LDFLAGS} \
  ${MKLLDFLAGS} \
...
echo "Built build/test_laplace_tiny_open"
ldd build/test_laplace_tiny_open | egrep "not found|hdf5|mpi|mkl|qcd|z" || true
