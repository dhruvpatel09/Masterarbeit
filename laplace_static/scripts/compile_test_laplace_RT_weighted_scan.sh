...
rm -f \
  build/test_laplace_RT_weighted_scan \
  build/test_laplace_RT_weighted_scan.o \
  build/DistEigvecsHdf5Reader_RT_weighted.o

mpicc ${CFLAGS} \
  -c ${EVECH5}/src/DistEigvecsHdf5Reader.c \
  -o build/DistEigvecsHdf5Reader_RT_weighted.o

mpicc ${CFLAGS} \
  -c src/test_laplace_RT_weighted_scan.c \
  -o build/test_laplace_RT_weighted_scan.o

mpicxx -o build/test_laplace_RT_weighted_scan \
  build/test_laplace_RT_weighted_scan.o \
  build/DistEigvecsHdf5Reader_RT_weighted.o \
  ${LDFLAGS} \
  ${H5LDFLAGS} \
...
echo "Built build/test_laplace_RT_weighted_scan"
ldd build/test_laplace_RT_weighted_scan | \
