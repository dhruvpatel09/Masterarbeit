...
for tau in [4, 6]:
    ...
    for dx in sorted(df["dx"].unique()):
        ...
        # Important: same ensemble name for L, LS, S so pyerrors keeps correlations.
        ens = f"Em1_R4_tau{tau}_dx{dx}_dy0_axisavg"
...
out = Path("results/laplace_clover_R4_axisavg_rho_S_axial_tau4_tau6_pyerrors_corr.csv")
