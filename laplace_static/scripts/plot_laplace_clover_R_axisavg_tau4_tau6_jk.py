...
if len(sys.argv) != 2:
    raise SystemExit("Usage: plot_laplace_clover_R_axisavg_tau4_tau6_jk.py R")
...
s4 = pd.read_csv(f"results/laplace_clover_probe_R{R}tau4_axes0-2_t0avg48_n1-100_jk_ratio_summary.csv")
s6 = pd.read_csv(f"results/laplace_clover_probe_R{R}tau6_axes0-2_t0avg48_n1-100_jk_ratio_summary.csv")
...
ax.set_title(rf"Axis-averaged tau stability of $\rho_S$, $R/a={R}$, $N_{{\rm cfg}}=100$")
ax.grid(True, alpha=0.3)
ax.legend()

fig.tight_layout()
out = outdir / f"laplace_clover_R{R}_rho_S_axis_profile_tau4_tau6_t0avg48_n100_axisavg_jk_ratio_compare.png"
fig.savefig(out, dpi=300)
plt.close(fig)

comp_out = f"results/laplace_clover_R{R}_rho_S_axis_tau4_tau6_axisavg_jk_ratio_compare_t0avg48_n100.csv"
...
print("max |pull| inside source interval dx=0..R =",
      comp[(comp["dx"] >= 0) & (comp["dx"] <= R)]["pull"].abs().max())
