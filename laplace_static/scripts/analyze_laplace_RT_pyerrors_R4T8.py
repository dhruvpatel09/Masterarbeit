...
JK_FILE = (
    RESULTS /
    "laplace_Veff_R0-4_T1-7_n1-100_jackknife.csv"
)

OUT_L = (
    RESULTS /
    "spatial_avg_RT_R0-4_T1-8_n1-100_pyerrors.csv"
)
...
OUT_CMP = (
    RESULTS /
    "laplace_Veff_R0-4_T1-7_n1-100_error_comparison.csv"
)

ENSEMBLE = "Em1p4_laplace_R4T8"
S_VALUE = 2.0
...
def main():
    ...
    plt.xlabel("T/a")
    plt.ylabel("Re L(R,T)")
    plt.title("Laplace correlator: Gamma-method uncertainties")
    plt.xticks(EXPECTED_T)
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(
        FIGDIR / "laplace_ReL_vs_T_pyerrors_R4T8.png",
        dpi=200,
    )
    plt.close()
    ...
    plt.title("Laplace effective potential: Gamma method")
    plt.xticks(EXPECTED_T[:-1])
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(
        FIGDIR / "laplace_Veff_vs_T_pyerrors_R4T8.png",
        dpi=200,
    )
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(
        FIGDIR / "laplace_Veff_vs_R_pyerrors_R4T8.png",
        dpi=200,
    )
    plt.close()
    ...
    plt.title("Relative Gamma-method uncertainty of Veff")
    plt.xticks(EXPECTED_T[:-1])
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(
        FIGDIR /
        "laplace_Veff_relative_error_pyerrors_R4T8.png",
        dpi=200,
    )
    plt.close()
    ...
    for R in EXPECTED_R[1:]:
        ...
        plt.savefig(
            FIGDIR /
            f"laplace_Veff_pyerrors_zoom_R{R}_R4T8.png",
            dpi=200,
        )
        plt.close()
        ...
        plt.savefig(
            FIGDIR /
            f"laplace_Veff_methods_R{R}_R4T8.png",
            dpi=200,
        )
        plt.close()
