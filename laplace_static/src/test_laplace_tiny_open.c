...
int main(int argc, char **argv) {
    ...
    if (numprocs != 1) {
        if (myid == 0) {
            fprintf(stderr, "ERROR: validation requires exactly one MPI rank\n");
        }
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    }
    ...
    qcd_spinorComponent3d *v_src = calloc((size_t)nvecs, sizeof(*v_src));
    qcd_spinorComponent3d *v_sink = calloc((size_t)nvecs, sizeof(*v_sink));
    ...
    for (int i = 0; i < nvecs; i++) {
        if (qcd_initSpinorComponent3d(&v_src[i], &geo) ||
            qcd_initSpinorComponent3d(&v_sink[i], &geo)) {
            fprintf(stderr, "ERROR: qcd_initSpinorComponent3d failed at i=%d\n", i);
            MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
        }
    }
    ...
    qcd_complex_16 combined_sum = {0.0, 0.0};
    long combined_nsrc = 0;
    double max_pair_conjugacy_abs = 0.0;
    double max_axis_avg_abs_im = 0.0;

    for (int axis = 0; axis < 3; axis++) {
        qcd_complex_16 axis_sum = {0.0, 0.0};
        ...
        for (int x = 0; x < L[1]; x++) {
            for (int y = 0; y < L[2]; y++) {
                for (int z = 0; z < L[3]; z++) {
                    ...
                    const qcd_complex_16 value = pair_correlator(
                    ...
                    axis_sum = qcd_CADD(axis_sum, value);
                    axis_nsrc++;

                    const double residual_re = value.re - reverse.re;
                    const double residual_im = value.im + reverse.im;
                    const double residual_abs = hypot(residual_re, residual_im);
        ...
        const qcd_complex_16 axis_avg = {
            axis_sum.re / (double)axis_nsrc,
            axis_sum.im / (double)axis_nsrc
        };
        if (fabs(axis_avg.im) > max_axis_avg_abs_im) {
            max_axis_avg_abs_im = fabs(axis_avg.im);
        }

        combined_sum = qcd_CADD(combined_sum, axis_sum);
        combined_nsrc += axis_nsrc;

        if (myid == 0) {
            printf(
                "PAIR axis=%s source=0,0,0 Re=%+.16e Im=%+.16e\n",
                axis_name[axis],
                origin_pair.re,
                origin_pair.im
            );
            printf(
                "AXIS axis=%s Nsrc=%ld Re=%+.16e Im=%+.16e\n",
                axis_name[axis],
                axis_nsrc,
                axis_avg.re,
                axis_avg.im
    ...
    const qcd_complex_16 combined_avg = {
        combined_sum.re / (double)combined_nsrc,
        combined_sum.im / (double)combined_nsrc
    };
    const double combined_im_relative = (
        fabs(combined_avg.im) / fmax(1.0, fabs(combined_avg.re))
    );

    if (myid == 0) {
        printf(
            "COMBINED Nsrc=%ld Re=%+.16e Im=%+.16e\n",
            combined_nsrc,
            combined_avg.re,
            combined_avg.im
        );
        printf(
            ...
            "max_axis_avg_abs_im=%+.16e combined_rel_im=%+.16e\n",
            max_pair_conjugacy_abs,
            max_axis_avg_abs_im,
            combined_im_relative
        );
        fflush(stdout);
    }
    ...
    free(tau);
    for (int i = 0; i < nvecs; i++) {
        qcd_destroySpinorComponent3d(&v_src[i]);
        qcd_destroySpinorComponent3d(&v_sink[i]);
    }
    free(v_src);
    free(v_sink);
